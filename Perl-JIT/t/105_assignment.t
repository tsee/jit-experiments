...
my @tests = (
  ...
  { name   => 'assign expression',
    ...
    input  => [-42], },
  { name   => 'assign non-jittable',
    func   => build_jit_test_sub('$a', 'my $x = srand($a)', '$x + $a'),
    opgrep => [{ name => 'add' }, { name => 'sassign' }],
    input  => [21], },
  { name   => 'assign targmy non-jittable',
    func   => build_jit_test_sub('$a', 'my $x; $x = srand($a)', '$x + $a'),
    opgrep => [{ name => 'add' }],
    input  => [21], },
